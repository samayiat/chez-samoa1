...
## Commit workflow
> **CHANGED 2026-07-15 — the repo is the source of truth. There is no more ZIP.**
> A "commit" is a git commit. The **built** `culinary-dash.html` is *gitignored*
> and ships as a **GitHub Release asset** (`gh release create vX.Y.Z culinary-dash.html`),
> tagged per delivery — never committed, because it inlines all 26 art blobs and
> would store ~900KB of unchanged art on every commit. See DECISIONS.md.
> `CHANGELOG.md` still gets an entry per commit. Code gets commented as it's written. Patches are batched from a manifest, not applied one-off.
