...
d = json.load(open("/home/claude/work/ing_ops.json"))
...
def render(cid, depth=0, W=320, H=180):
    ...
    sa, sd = 1.0, 1.0          # the CAMERA scale. The old recorder threw setTransform away entirely
                               # ("ident"), so every render silently ignored zoom AND pan.
    for op in ALL[cid]:
        k = op[0]
        if k == "save": stack.append((tx,ty,rot,sa,sd))
        elif k == "restore":
            if stack: tx,ty,rot,sa,sd = stack.pop()
        elif k == "tr": tx += sa*op[1]; ty += sd*op[2]
        elif k == "rot": rot += op[1]
        elif k == "ident": tx=ty=0.0; rot=0.0; sa=sd=1.0
        elif k == "xform": sa,sd,tx,ty = op[1],op[4],op[5],op[6]; rot=0.0
        elif k == "clr":
            x,y,w,h = [int(round(v)) for v in op[1:5]]
            img.paste((0,0,0,0), (x,y,x+w,y+h))
        elif k == "r":
            x,y,w,h,fs,ga = op[1],op[2],op[3],op[4],op[5],op[6]
            gco = op[7] if len(op) > 7 else None
            ...
            if abs(rot) > 1e-6:
                ...
                x2,y2 = sa*x+tx, sd*y+ty
                xi,yi,wi,hi = int(round(x2)), int(round(y2)), int(round(sa*w)), int(round(sd*h))
                if wi <= 0 or hi <= 0: continue
                sub = Image.new("RGBA", (wi,hi), c)
                lay = Image.new("RGBA", (W,H), (0,0,0,0)); lay.paste(sub, (xi,yi))
                if gco == "source-atop":
                    # the fill only lands where pixels already exist -- that IS how tinted() masks a
                    # tint to the sprite instead of painting a square over it
                    lay.putalpha(Image.fromarray(
                        (np.asarray(lay.split()[3]).astype(int) * (np.asarray(img.split()[3]).astype(int) > 0)).astype("uint8")))
                img = Image.alpha_composite(img, lay)
        elif k == "img":
            sid, x, y, w, h, ga = op[1], op[2], op[3], op[4], op[5], op[6]
            if sid is None: continue
            if isinstance(sid, str) and sid.startswith("uri:data:image/png;base64,"):
                import base64, io
                src = Image.open(io.BytesIO(base64.b64decode(sid.split(",",1)[1]))).convert("RGBA")
            else:
                src = render(sid, depth+1)
            if x is None: x = 0
            if y is None: y = 0
            lay = Image.new("RGBA", (W,H), (0,0,0,0))
            dw = sa*(w if w else src.width); dh = sd*(h if h else src.height)
            if int(round(dw)) != src.width or int(round(dh)) != src.height:
                try: src = src.resize((max(1,int(round(dw))), max(1,int(round(dh)))), Image.NEAREST)
                except: pass
            lay.paste(src, (int(round(sa*x+tx)), int(round(sd*y+ty))), src)
