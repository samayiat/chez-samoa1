...
;(function(){
  ...
  const R=[];
  const rows=[["jab",HIT.jab],["cross",HIT.jab+(MOVE_W.cross||0)],["roundhouse",HIT.jab+(MOVE_W.roundhouse||0)],
              ["jab KO",HIT.jab+HIT_KO],["roundhouse KO",HIT.jab+(MOVE_W.roundhouse||0)+HIT_KO],
              ["+3 bodies",HIT.jab+(MOVE_W.roundhouse||0)+HIT_KO+HIT_BODY*3]];
  for(const [n,w] of rows){
    const cw=Math.max(0,Math.min(HIT_MAX,w));
    R.push(n.padEnd(15)+" w="+cw.toFixed(2).padStart(5)+"  ->  "+String(hapticFor(cw)).padStart(2)+"ms   shake+"+(cw*SHAKE_PER_W).toFixed(2));
  }
  globalThis.__R=R;
})();`;
vm.runInContext(script+"\n"+probe, vm.createContext(sb), {filename:"dev.js"});
console.log("  every channel off ONE weight:\n");
