...
;(function(){
  ...
  const states=["idle","walk","groom","sleep"];
  for(let i=0;i<4;i++){
    drawOneCat({x:14+i*22, y:14, dir:1, pal:"tux", state:states[i], tail:0.7, blink:0, gone:false});
    drawOneCat({x:14+i*22, y:30, dir:1, pal:"orange", state:states[i], tail:0.7, blink:0, gone:false});
    drawOneCat({x:14+i*22, y:50, dir:-1, pal:"tux", state:states[i], tail:0.7, blink:0, gone:false});
    drawOneCat({x:14+i*22, y:66, dir:-1, pal:"orange", state:states[i], tail:0.7, blink:0, gone:false});
  }
})();`;
...
fs.writeFileSync("/home/claude/work/cat_ops.json", JSON.stringify({ main: MAIN.__id, all: dump }));
