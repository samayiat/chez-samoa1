...
const sb=Object.assign({},sandbox); sb.globalThis=sb; sb.navigator={userAgent:"node",vibrate:()=>true};
const p=`;(function(){
  startCampaign(); startDay(); devSeedBrawl();
  const R=[];
  const rows=[["scuff",HIT.scuff],["jab",HIT.jab],["cross",HIT.jab+(MOVE_W.cross||0)],
    ["roundhouse",HIT.jab+(MOVE_W.roundhouse||0)],["special",HIT.heavy],
    ["jab KO",HIT.jab+HIT_KO],["roundhouse KO",HIT.jab+(MOVE_W.roundhouse||0)+HIT_KO],
    ["stumble (you)",HIT.stumble],["+3 bodies KO",HIT.jab+(MOVE_W.roundhouse||0)+HIT_KO+HIT_BODY*3]];
  brawl.drinks=0; brawl.wastedT=0;
  R.push("  SOBER"); 
  for(const [n,w] of rows) R.push("   "+n.padEnd(15)+"w="+w.toFixed(2).padStart(5)+"   "+JSON.stringify(hapticFor(w,hapticMult())));
  brawl.drinks=WASTED_AT+WASTED_DEPTH-1; brawl.wastedT=WASTED_TIME;
  R.push("  WASTED (x"+hapticMult().toFixed(2)+")");
  for(const [n,w] of rows) R.push("   "+n.padEnd(15)+"w="+w.toFixed(2).padStart(5)+"   "+JSON.stringify(hapticFor(w,hapticMult())));
  R.push("  DAY (a tap on the shoulder)");
  for(const k of Object.keys(NOTIFY)) R.push("   "+k.padEnd(15)+JSON.stringify(NOTIFY[k]));
  globalThis.__R=R;
})();`;
vm.runInContext(script+"\n"+p, vm.createContext(sb), {filename:"dev.js"});
sb.__R.forEach(l=>console.log(l));